class PasswordResetsController < ApplicationController
  ...
  def create
    ...
    if user
      user.generate_password_reset_token!
			...
			#puts "DBG user: result_user=#{user.inspect}"
  ...
  def edit
		...
		#puts "DBG id: result_id=#{params[:id].inspect}"			
		@user = User.find_by(password_reset_token: params[:id])		
		puts "DBG user: user=#{@user.inspect}"
		if @user
    else
  ...
  def update
    ...
    if @user && @user.update_attributes(user_params)
      ...
      redirect_to posts_path, success: "Password updated."
    else
